...
std::vector<ObjDetector::DetectionInfo> ObjDetector::detect(cv::Mat& frame){
	...
	if (params_.isInit()){
		...
		putText(currFrame, "rois.size() = " + num2str(rois.size()), Point(200,250),
				CV_FONT_HERSHEY_PLAIN, 2.0, cv::Scalar(0, 255, 0));
...
//				cv::rectangle(tmp, r, cv::Scalar(0, 255, 0), 2);
				cv::rectangle(currFrame, r, cv::Scalar(0, 255, 0), 2);
			}
			//cv::imshow("Stage 1", tmp);
		}

		string cascade("cascadeMinWin: "+num2str(params_.cascadeMinWin.width)+", "+num2str(params_.cascadeMinWin.height));
		putText(currFrame, cascade, Point(200,350),
				CV_FONT_HERSHEY_PLAIN, 2.0, cv::Scalar(0, 255, 0));
